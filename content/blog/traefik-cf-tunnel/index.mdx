...
저는 계속해서 홈랩을 처음 운영하고자하는 누군가의 노력이, 혹은 제 자신이 다시 설정하는 일이 있더라고 최대한 간단하면서 높은 활용도를 가진 설정을 원했습니다.

단지 하나의 raspberrypi에서 동작하다가 proxmox를 이용하도록 확장되어도 문제 없는 그러한 간단한 설정을 추구하였습니다.

이제 여기에 그 답이 있습니다.

저는 현재 “어떠한” 랜덤한 컴퓨터에서 “인터넷과 도커” 만 된다면 자신만의 프로젝트를 인터넷에 공용으로 공유할 수 있는 방법에 대해서 글을 작성하고 있습니다.

이를 달성하기 위해 지금 속해있는 환경에 네트워크 설정 권한을 요구하지 않을 것이고, 포트포워딩도 필요하지 않을 것입니다.

또한 처음 홈랩 작업을 수행할때 큰 허들이 되는 SSL 인증서를 발급 받는 과정 역시 존재하지 않습니다.

여기에 더해 더 이상 공인 아이피가 회전하는 것을 걱정하지 않아도 괜찮습니다.

우리는 홈랩을 구축하고 있습니다, 그런 우리에게 “안정적인 고정 IP”는 사치품입니다.

이제 더 이상 이 문제를 고민하지 마세요, ddns 등의 어려운 설정이 필요하지 않고 그냥 인터넷 접근이 필요한 전부입니다.

이런 이점을 가져오기 위해 cloudflare tunnel를 사용할 것입니다.

기존에 “포트포워딩된 공인 아이피”에 설정해주었던 wildcard DNS record 대신 cloudflare tunnel을 타겟으로 하는 레코드를 추가하고 traefik과 cf tunnel를 연결하여 모든 것을 동작하게 할 것입니다.
